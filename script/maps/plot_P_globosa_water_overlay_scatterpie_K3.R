...
structure_k3_file <- file.path(repo_root, "figure", "STRUCTURE_individual_allK", "STRUCTURE_K03_individual_mean.csv")
...
output_png <- file.path(repo_root, "figure", "P_globosa_water_overlay_scatterpie_K3.png")
...
read_structure_k3 <- function(path) {
  readr::read_csv(path, show_col_types = FALSE) %>%
    group_by(population_label) %>%
    summarise(
      cluster_1 = mean(cluster_1, na.rm = TRUE),
...
structure_k3 <- read_structure_k3(structure_k3_file)
...
point_xy <- site_sf_3857 %>%
  ...
  left_join(structure_k3, by = c("point_id" = "population_label")) %>%
  mutate(
    X_offset = X + dx,
    Y_offset = Y + dy
  )
...
if (any(is.na(point_xy$cluster_1) | is.na(point_xy$cluster_2) | is.na(point_xy$cluster_3))) {
  stop("K = 3 STRUCTURE values are missing for one or more Wen labels.")
}
...
p <- ggplot() +
  ...
  scatterpie::geom_scatterpie(
    ...
    cols = c("cluster_1", "cluster_2", "cluster_3"),
    color = "black",
    linewidth = 0.2,
    alpha = 1,
    show.legend = FALSE
  ...
  ggokabeito::scale_fill_okabe_ito(
    breaks = c("cluster_1", "cluster_2", "cluster_3"),
