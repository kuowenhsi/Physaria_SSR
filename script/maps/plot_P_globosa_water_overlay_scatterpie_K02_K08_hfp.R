...
hfp_states_file <- file.path(repo_root, "data", "P_globosa_map", "USA_adm1.shp")
hfp_file <- file.path(repo_root, "data", "hfp_2020_100m_p_globosa_water_overlay_extent.tif")
...
read_table_1 <- function(path) {
  ...
  readr::read_csv(I(data_lines), show_col_types = FALSE) %>%
    ...
    mutate(
      species_name = str_replace_all(species_name, "[â€œâ€]", ""),
...
prepare_hfp_overlay <- function(path, map_bbox, aggregate_fact = 3, alpha = 0.42) {
  hfp <- terra::rast(path)
  terra::NAflag(hfp) <- 65535
  hfp[hfp > 50000] <- NA
  hfp <- hfp / 1000
  hfp_3857 <- terra::project(hfp, "EPSG:3857", method = "near")
  hfp_crop <- terra::crop(
    hfp_3857,
    terra::ext(
      as.numeric(map_bbox["xmin"]),
      as.numeric(map_bbox["xmax"]),
      as.numeric(map_bbox["ymin"]),
      as.numeric(map_bbox["ymax"])
    )
  )
  if (aggregate_fact > 1) {
    hfp_crop <- terra::aggregate(hfp_crop, fact = aggregate_fact, fun = mean, na.rm = TRUE)
  }

  hfp_df <- terra::as.data.frame(hfp_crop, xy = TRUE, na.rm = TRUE)
  names(hfp_df)[3] <- "hfp_score"
  hfp_df %>%
    mutate(alpha = alpha)
}
...
hfp_df <- prepare_hfp_overlay(hfp_file, map_bbox)
...
state_sf <- st_read(hfp_states_file, quiet = TRUE) %>%
  filter(NAME_1 %in% c("Indiana", "Kentucky", "Tennessee", "Illinois")) %>%
  mutate(
    state_fill = recode(
      NAME_1,
      "Illinois" = "#f2f2f2",
...
for (k in k_values) {
  ...
  p <- ggplot() +
    ...
    geom_raster(
      data = hfp_df,
      aes(x = x, y = y, fill = hfp_score, alpha = alpha)
    ) +
    scale_alpha_identity() +
    scale_fill_gradientn(
      colours = c("#f7f3df", "#e7c86e", "#d88c3d", "#c2472b", "#7f0000"),
      values = scales::rescale(c(0, 5, 15, 30, 50)),
      limits = c(0, 50),
  ...
  ggsave(
    file.path(repo_root, "figure", sprintf("P_globosa_water_overlay_scatterpie_K%02d_hfp.png", k)),
    plot = p,
    width = 10,
    height = 8,
    dpi = 900
